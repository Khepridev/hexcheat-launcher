    ...
    if (isset($_POST['action']) && $_POST['action'] === 'upload_app_exe') {
        ...
        if (isset($_POST['app_id']) && isset($_FILES['app_exe_file']) && $_FILES['app_exe_file']['error'] === 0) {
            ...
            if (move_uploaded_file($fileTmpName, $uploadPath)) {
                // Veritabanına kaydet
                $conn = dbConnect();
                $stmt = $conn->prepare("INSERT INTO apps (app_name, app_path, app_size, app_hash, app_data_id) VALUES (?, ?, ?, ?, ?)");
                $stmt->bind_param("ssisi", $fileName, $relativePath, $fileSize, $fileHash, $appId);
                ...
                if ($stmt->execute()) {
                    ...
                    if (saveManifest($manifest)) {
                        ...
                        // Veritabanı kaydı başarılı ama manifest güncellenemedi
                        echo json_encode(['success' => true, 'warning' => 'Dosya veritabanına eklendi fakat manifest.json güncellenemedi']);
                    }
                } else {
            ...
            if (move_uploaded_file($fileTmpName, $physicalPath)) {
                ...
                if ($stmt->execute()) {
                    ...
                    if (saveManifest($manifest)) {
                        ...
                        // Veritabanı kaydı başarılı ama manifest güncellenemedi
                        echo json_encode(['success' => true, 'warning' => 'Alt dosya veritabanına eklendi fakat manifest.json güncellenemedi']);
                    }
                } else {
                    echo json_encode(['error' => 'Veritabanı hatası: ' . $conn->error]);
                }
        ...
        switch ($data['action']) {
            ...
            case 'edit_menu_item':
                if (isset($data['menu_id']) && isset($data['menu_lang'])) {
                    ...
                    // Diğer dildeki menüyü güncelle
                    foreach ($manifest['translations'][$otherLang]['menuItems'] as $key => &$item) {
                        if ((int)$item['id'] === $menuId) {
                            $manifest['translations'][$otherLang]['menuItems'][$key]['url'] = $data['url'];
                            break;
                        }
                    }
            ...
            case 'delete_file':
                if (isset($data['file_id'])) {
                    ...
                    // Önce dosyayı veritabanından sorgula
                    ...
                    if ($result->num_rows > 0) {
                        ...
                        $filePath = '../' . $file['file_url']; // Dosya yolu
                        
                        // Dosyayı veritabanından sil
                        $deleteStmt = $conn->prepare("DELETE FROM files WHERE file_id = ?");
                        $deleteStmt->bind_param("i", $fileId);
                        $success = $deleteStmt->execute();
                        $deleteStmt->close();
                        
                        // Eğer veritabanından silindiyse, dosya sisteminden de sil
                        if ($success && file_exists($filePath)) {
                            unlink($filePath); // Dosyayı fiziksel olarak sil
                        }
                        
                        if ($success) {
                            echo json_encode(['success' => true]);
                        } else {
                            echo json_encode(['error' => 'Dosya silinirken bir hata oluştu: ' . $conn->error]);
                        }
                    } else {
            ...
            case 'add_app':
                if (isset($data['name']) && isset($data['icon']) && isset($data['mainPath']) && isset($data['version'])) {
                    $newId = isset($manifest['apps']) ? count($manifest['apps']) + 1 : 1;
                    ...
                    // MySQL'e de ana uygulama kaydını ekleyelim
                    $conn = dbConnect();
                    $stmt = $conn->prepare("INSERT INTO apps (app_name, app_path, app_size, app_hash, app_data_id) VALUES (?, ?, ?, ?, ?)");
                    // Ana uygulama kaydı olarak burada boyut 0, hash boş string olarak kaydediyoruz
                    $fileSize = 0;
                    $fileHash = '';
                    $appName = $data['name'];
                    $appPath = $data['mainPath'];
                    $stmt->bind_param("ssisi", $appName, $appPath, $fileSize, $fileHash, $newId);
                    $stmt->execute();
                    $stmt->close();
                    $conn->close();
            ...
            case 'edit_app':
                if (isset($data['app_id']) && isset($data['name']) && isset($data['icon']) && isset($data['mainPath']) && isset($data['version'])) {
                    ...
                    // Eski ana yolu kaydet
                    $oldMainPath = '';
                    
                    foreach ($manifest['apps'] as &$app) {
                        if ((int)$app['id'] === $appId) {
                            $oldMainPath = $app['mainPath'];
                            $app['name'] = $data['name'];
                            $app['icon'] = $data['icon'];
                    ...
                    if ($updated) {
                        // MySQL'deki ana uygulama kaydını da güncelleyelim
                        $conn = dbConnect();
                        // Eski ana yola göre ana uygulama kaydını bulup güncelle
                        $stmt = $conn->prepare("UPDATE apps SET app_name = ?, app_path = ? WHERE app_path = ? AND app_data_id = ? AND app_size = 0");
                        $appName = $data['name'];
                        $newPath = $data['mainPath'];
                        $stmt->bind_param("sssi", $appName, $newPath, $oldMainPath, $appId);
                        $stmt->execute();
                        $stmt->close();
                        $conn->close();
            ...
            case 'delete_app':
                if (isset($data['id'])) {
                    ...
                    // Veritabanından bu uygulamaya ait tüm dosyaları silelim
                    $conn = dbConnect();
                    $stmt = $conn->prepare("DELETE FROM apps WHERE app_data_id = ?");
                    ...
                    // Uygulamayı filtrele
                    $manifest['apps'] = array_values(array_filter(
                        $manifest['apps'],
                        function($app) use ($appId) {
                            return (int)$app['id'] !== $appId;
                        }
                    ));
